...
router.route('/:messageID')
  .get((req, res, next) => {
    ...
    State.findOne({ 'messageID': messageID }).then((doc) => {
      if (doc) {
        ...
        const x = {
          'messageID': messageID,
          tfState: JSON.stringify({ 'serial': 0, 'version': 1, 'modules': [{ 'path': ['root'], 'resources': {}, 'outputs': {} }] })
        }
        const s = new State(x)
        s.save(x).then((p) => {
          res.send(JSON.parse(p.tfState))
        }
        )
      }
  ...
  .post((req, res, next) => {
    ...
    return State.findOneAndUpdate({ 'messageID': messageID }, { '$set': { messageID, tfState: JSON.stringify(tfState) } }, { 'upsert': true, new: true }).then((doc) => {
      if (doc) {
        if (req.query.ID) {
