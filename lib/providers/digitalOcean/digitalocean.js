...
class DOProvider {
  ...
  createVariablesJSON() {
    ...
    fs.writeFile(path.join(this.request.jobDirectory, "variables.tf.json"), JSON.stringify({ "variable": this.request.tfVariables }, null, 4), (err) => {
      if (err)
  ...
  createMainJSON() {
    ...
    fs.writeFile(path.join(this.request.jobDirectory, "main.tf.json"), JSON.stringify(this.request.tfMain, null, 4), (err) => {
      if (err)
