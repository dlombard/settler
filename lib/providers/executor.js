...
class Executor {
  ...
  execute(msg) {
    ...
    if (_toUpper(action) == 'DELETE') {
      return this.deleteSettlement(msg)
    }
    else if (_toUpper(action) == 'CREATE')
      return this.createSettlement(msg)
  }

  deleteSettlement(msg) {
    logger.warn('delete')

  }
  createSettlement(msg) {
    let provider;
    const request = JSON.parse(msg.content.toString())
    const settlement = {
    ...
    return Settlement.findOneAndUpdate({ 'messageId': request.messageId }, settlement, { 'upsert': true, 'new': true })
      .then((r) => {
        ...
        switch (_toUpper(request.data.provider.type)) {
          case 'DIGITALOCEAN':
            provider = new DigitalOcean(request, msg.properties.messageId)
            break
          case 'AWS_LIGHTSAIL':
            provider = new AWS_Lightsail(request, msg.properties.messageId)
            break
        }
        if (provider) {
